...
const I18N = {
  en:{
    ...
    "f.faculty":"Faculty","f.autofill":"(auto-filled)","f.program":"Degree Program","f.semester":"Academic Level","f.year":"Year of Study","f.cgpa":"CGPA","f.optional":"(optional)","f.level":"Degree","f.levelph":"Select your degree","f.skills":"Skills","f.hobbies":"Hobbies & Interests",
  ...
  ar:{
    ...
    "f.faculty":"الكلية","f.autofill":"(معبأ تلقائيًا)","f.program":"البرنامج الدراسي","f.semester":"المستوى الأكاديمي","f.year":"سنة الدراسة","f.cgpa":"المعدل التراكمي","f.optional":"(اختياري)","f.level":"الدرجة العلمية","f.levelph":"اختر الدرجة العلمية","f.skills":"المهارات","f.hobbies":"الهوايات والاهتمامات",
...
let state = {
  department: null,
  gender: null,
  flow: 'student',   // 'student' | 'council' | 'competition'
};

/* Flow-aware wizard navigation.
   • student / council : choose faculty → choose council (gender) → form
   • competition       : choose council (gender) → choose faculty → form   */
function isComp(){ return state.flow==='competition' && !!(state.competition && state.competition.id); }
function afterDept(){ if(!state.department) return; if(isComp()){ go('register'); } else { go('gender'); } }
function afterGender(){ if(!state.gender) return; if(isComp()){ go('departments'); } else { go('register'); } }
function backFromDept(){ if(isComp()){ go('compdetail'); } else { go('home'); } }
function backFromGender(){ if(isComp()){ go('compdetail'); } else { go('departments'); } }
function backFromRegister(){ if(isComp()){ go('departments'); } else { go('gender'); } }
function backAcademic(){ if(isComp()){ go('departments'); } else { prevStep(2); } }
function startStudentReg(){ state.flow='student'; state.mode=''; state.competition=null; state.department=null; state.gender=null; clearRegForm(); go('departments'); }
...
function renderDepartments(){
  ...
  DEPARTMENTS.forEach((d) => {
    ...
    btn.onclick = () => { state.department = d; renderDepartments(); document.getElementById('deptContinue').disabled = false; setTimeout(function(){ afterDept(); }, 280); };
...
function selectGender(g){
  ...
  setTimeout(function(){ afterGender(); }, 300);
}
...
/* Short-lived caches so repeat views (home popup, competitions page, analytics)
   don't re-hit the API every time — keeps the site feeling fast.
   Public list carries images (for cards/popup); admin list is image-free (fast). */
var _pubComps=null,_pubAt=0, _admComps=null,_admAt=0;
async function getCompetitions(force){            // public — includes images
  var now=Date.now();
  if(!force && _pubComps && (now-_pubAt)<20000) return _pubComps;
  var r = await api('/api/competitions');
  _pubComps = r.competitions || []; _pubAt = now;
  return _pubComps;
}
async function getAdminCompetitions(force){       // admin — no images, has entry counts
  var now=Date.now();
  if(!force && _admComps && (now-_admAt)<15000) return _admComps;
  var r = await api('/api/competitions', { headers: authHeaders() });
  _admComps = r.competitions || []; _admAt = now;
  return _admComps;
}
function invalidateComps(){ _pubComps=null; _pubAt=0; _admComps=null; _admAt=0; }
...
async function openCompetition(id){
  ...
  var media = c.image
    ? '<div class="cd-media"><img src="'+c.image+'" alt=""></div>'
    : '<div class="cd-media"><div class="comp-noimg">🏆</div></div>';
  var foot;
  if(c.status==='open'){
    foot = '<button class="btn-primary" onclick="startCompetitionEntry('+c.id+')">'+tr('Register for this competition','سجّل في هذه المسابقة')+' →</button>';
  } else if(c.status==='soon'){
    foot = '<span class="comp-msg">'+tr('Registration opens soon, in shā’ Allah.','التسجيل يُفتح قريبًا بإذن الله.')+'</span>';
  } else {
    foot = '<span class="comp-msg closed">'+tr('Registration for this competition is closed.','التسجيل في هذه المسابقة مغلق.')+'</span>';
  }
  box.innerHTML = '<button class="btn-outline sm" onclick="go(\'competitions\')">← '+tr('Back to competitions','رجوع للمسابقات')+'</button>'
    + '<div class="comp-detail-2col">'
    + '<div class="cd-text">'
    + (c.category ? '<span class="comp-cat">'+esc(c.category)+'</span>' : '')
...
function startCompetitionEntry(id){
  ...
  clearRegForm();
  go('gender');                                      // competition: council (gender) first, then faculty
}
...
async function renderAdminCompetitions(){
  ...
  box.innerHTML = '<div class="empty-card">'+tr('Loading…','جارٍ التحميل…')+'</div>';
  var comps = [];
  try{ comps = await getAdminCompetitions(true); }
  ...
  window._acomps={}; comps.forEach(function(c){ window._acomps[c.id]=c; });
  var totalReg=comps.reduce(function(s,c){return s+(c.entries||0);},0);
  var openN=comps.filter(function(c){return c.status==='open';}).length;
  var soonN=comps.filter(function(c){return c.status==='soon';}).length;
  var closedN=comps.filter(function(c){return c.status==='closed';}).length;
  var tiles='<div class="mini-tiles" style="margin:6px 0 14px">'
    +'<div class="mini-tile"><b>'+comps.length+'</b><span>'+tr('Competitions','المسابقات')+'</span></div>'
    +'<div class="mini-tile"><b>'+totalReg+'</b><span>'+tr('Registrations','المسجّلون')+'</span></div>'
    +'<div class="mini-tile"><b>'+openN+'</b><span>'+tr('Open','متاحة')+'</span></div>'
    +'<div class="mini-tile"><b>'+soonN+'</b><span>'+tr('Coming soon','قريبًا')+'</span></div>'
    +'<div class="mini-tile"><b>'+closedN+'</b><span>'+tr('Closed','مغلقة')+'</span></div></div>';
  var list = comps.length ? comps.map(function(c){
    var t=(lang==='ar'&&c.title_ar)?c.title_ar:c.title;
    return '<div class="rbac-row"><div class="rbac-info"><b>'+esc(t)+'</b> '+compStatusBadge(c.status)
      + (c.featured?' <span class="rbac-badge s-open">★ '+tr('Featured','مميّزة')+'</span>':'')
      ...
      + '<button class="btn-outline sm" onclick="toggleFeatured('+c.id+')">'+(c.featured?('★ '+tr('Unfeature','إلغاء التمييز')):('☆ '+tr('Feature on home','تمييز بالرئيسية')))+'</button>'
      + '<button class="btn-outline sm" onclick="editCompetition('+c.id+')">'+tr('Edit','تعديل')+'</button>'
      ...
      + '</div><div class="edit-box" id="edit-'+c.id+'"></div><div class="entries-box" id="entries-'+c.id+'"></div></div>';
  }).join('') : '<div class="empty-card">'+tr('No competitions yet. Add one above.','لا توجد مسابقات بعد. أضِف واحدة بالأعلى.')+'</div>';
  box.innerHTML = '<div class="rbac-manage">'+form+tiles+'<div class="rbac-list">'+list+'</div></div>';
}
/* Edit an existing competition — does NOT affect people who already registered
   (entries live in a separate table) nor block new registrations. */
function editCompetition(id){
  var box=document.getElementById('edit-'+id); if(!box) return;
  if(box.getAttribute('data-open')==='1'){ box.innerHTML=''; box.setAttribute('data-open','0'); return; }
  box.setAttribute('data-open','1');
  var c=(window._acomps||{})[id]||{};
  box.innerHTML='<form class="mng-form col" style="margin-top:12px" onsubmit="saveCompetition(event,'+id+')">'
    +'<input id="ec_title_'+id+'" value="'+esc(c.title||'')+'" placeholder="'+tr('Title (English)','العنوان (إنجليزي)')+'">'
    +'<input id="ec_title_ar_'+id+'" value="'+esc(c.title_ar||'')+'" placeholder="'+tr('Title (Arabic)','العنوان (عربي)')+'">'
    +'<input id="ec_category_'+id+'" value="'+esc(c.category||'')+'" placeholder="'+tr('Category (optional)','التصنيف (اختياري)')+'">'
    +'<textarea id="ec_desc_'+id+'" rows="2" placeholder="'+tr('Description','الوصف')+'">'+esc(c.description||'')+'</textarea>'
    +'<textarea id="ec_reqs_'+id+'" rows="2" placeholder="'+tr('Requirements / conditions','الشروط والمتطلبات')+'">'+esc(c.requirements||'')+'</textarea>'
    +'<label class="fld-label">'+tr('Replace image (optional)','استبدال الصورة (اختياري)')+'</label>'
    +'<input type="file" id="ec_image_'+id+'" accept="image/*">'
    +'<div class="export-row"><button class="btn-teal" type="submit">'+tr('Save changes','حفظ التعديلات')+'</button>'
    +'<button class="btn-outline" type="button" onclick="editCompetition('+id+')">'+tr('Cancel','إلغاء')+'</button></div></form>';
}
async function saveCompetition(ev,id){
  if(ev) ev.preventDefault();
  var t=val('ec_title_'+id); if(!t){ alert(tr('Title is required.','العنوان مطلوب.')); return; }
  var body={ title:t, title_ar:val('ec_title_ar_'+id), category:val('ec_category_'+id), description:val('ec_desc_'+id), requirements:val('ec_reqs_'+id) };
  var image=await readImageCompressed('ec_image_'+id,900); if(image) body.image=image;
  try{ await api('/api/competitions/'+id,{method:'PATCH',headers:authHeaders(),body:JSON.stringify(body)}); invalidateComps(); renderAdminCompetitions(); }
  catch(e){ alert(tr('Could not save: ','تعذّر الحفظ: ')+(e&&e.message||'')); }
}
async function toggleFeatured(id){
  var c=(window._acomps||{})[id]||{}; var make=!c.featured;
  try{
    await api('/api/competitions/'+id,{method:'PATCH',headers:authHeaders(),body:JSON.stringify({featured:make})});
    invalidateComps();
  ...
  return new Promise(function(resolve){
    ...
    reader.onload=function(e){
      ...
      img.onload=function(){
        ...
        try{ resolve(cv.toDataURL('image/jpeg',0.78)); }catch(err){ resolve(e.target.result); }
      };
...
async function createCompetition(ev){
  ...
  var image = await readImageCompressed('cmp_image', 900);
  ...
  try{ await api('/api/competitions', { method:'POST', headers: authHeaders(), body: JSON.stringify(body) }); invalidateComps(); renderAdminCompetitions(); }
  catch(e){ alert(tr('Could not save: ','تعذّر الحفظ: ')+(e && e.message || '')); }
}
async function setCompStatus(id, status){
  try{ await api('/api/competitions/'+id, { method:'PATCH', headers: authHeaders(), body: JSON.stringify({ status:status }) }); invalidateComps(); }
  catch(e){ alert(e && e.message || 'Error'); }
}
async function deleteCompetition(id){
  if(!confirm(tr('Delete this competition? This cannot be undone.','حذف هذه المسابقة؟ لا يمكن التراجع.'))) return;
  try{ await api('/api/competitions/'+id, { method:'DELETE', headers: authHeaders() }); invalidateComps(); renderAdminCompetitions(); }
  catch(e){ alert(e && e.message || 'Error'); }
}
async function viewEntries(id){
  ...
  window._entries=window._entries||{}; window._entries[id]=entries;
  ...
  var rejected=entries.filter(function(e){return e.status==='rejected';}).length;
  ...
  var rows=entries.map(function(en){
    ...
    return '<div class="entry-row"><div class="rbac-info"><button class="entry-name-btn" onclick="showEntryDetail('+id+','+en.id+')">'+esc(en.name)+'</button> <span class="rbac-badge s-'+badge+'">'+esc(en.status)+'</span><span class="rbac-sub">'+esc(en.email||'')+(en.faculty?' · '+esc(facLabel(en.faculty)):'')+(en.phone?' · '+esc(en.phone):'')+'</span></div>'
      + '<div class="rbac-actions">'
  ...
  box.innerHTML = analytics+rows;
}
/* Registrant detail — reuses the registration modal. */
function showEntryDetail(cid, eid){
  var list=(window._entries||{})[cid]||[];
  var d=list.filter(function(x){return String(x.id)===String(eid);})[0]; if(!d) return;
  document.getElementById('regModalName').textContent=d.name||'—';
  var badge=d.status==='approved'?'open':(d.status==='rejected'?'closed':'draft');
  document.getElementById('regModalStatus').innerHTML='<span class="rbac-badge s-'+badge+'">'+esc(d.status)+'</span>';
  document.getElementById('regModalActions').innerHTML=
    (d.status!=='approved'?'<button class="btn-teal" onclick="setEntry('+d.id+',\'approved\','+cid+');closeRegModal()">'+tr('Approve','قبول')+'</button>':'')
    +(d.status!=='rejected'?'<button class="btn-outline" onclick="setEntry('+d.id+',\'rejected\','+cid+');closeRegModal()">'+tr('Reject','رفض')+'</button>':'')
    +'<button class="btn-outline" style="color:#b13a63;border-color:#f3c6d3" onclick="delEntry('+d.id+','+cid+');closeRegModal()">'+tr('Delete','حذف')+'</button>';
...
function exportCSV(filename, rows, cols){
  var header=cols.map(function(c){return csvCell(c[1]);}).join(',');
  var body=(rows||[]).map(function(r){ return cols.map(function(c){ var v=r[c[0]]; if(Array.isArray(v)) v=v.join('; '); return csvCell(v); }).join(','); }).join('\n');
  var csv='﻿'+header+'\n'+body;   // BOM so Excel reads UTF-8 (Arabic) correctly
  var blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});
  var url=URL.createObjectURL(blob);
  var a=document.createElement('a'); a.href=url; a.download=(filename||'export').replace(/[^\w\-]+/g,'_')+'.csv';
  document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
}
function exportEntries(id){
  var list=(window._entries||{})[id]||[]; var comp=(window._acomps||{})[id]||{};
...
function exportMembers(){
  ...
  exportCSV('council-members', window._members||[], cols);
}
...
function renderCouncilAddStep(){
  ...
  if(!state._cmDept){
    ...
    box.innerHTML = '<div class="formcard">'
      ...
      + '<p class="portal-sub" style="margin-top:12px">'+tr("Each faculty may have up to 3 male and 3 female members. Additions are approved by the Admin.",'لكل كلية بحدٍّ أقصى ٣ طلاب و٣ طالبات. تخضع الإضافات لاعتماد المشرف.')+'</p>'
...
async function renderCouncilMembers(){
  ...
  box.innerHTML='<div class="export-row"><button class="btn-outline sm" onclick="exportMembers()">⬇ '+tr('Export CSV (Excel)','تصدير CSV (إكسل)')+'</button></div>'
    +'<div class="rbac-list">'+members.map(function(m){
...
/* Shape the registration wizard for the active flow. Competitors register on a
   single combined page (personal + academic), with no social-accounts field and
   no skills step. */
function arrangeWizard(){
  var comp=isComp();
...
function goToStep(n){
  var comp=isComp();
  [1,2,3,4].forEach(i => { const _el=document.getElementById('formstep-'+i); if(_el) _el.style.display='none'; });
  if(comp){
    // Competitors: page 1 = personal + academic combined; page 2 = review.
    if(n>=2 && n<=3) n=4;
    if(n===4){ document.getElementById('formstep-4').style.display='block'; }
    else { n=1; document.getElementById('formstep-1').style.display='block'; document.getElementById('formstep-2').style.display='block'; }
  } else {
    const _t=document.getElementById('formstep-'+n); if(_t) _t.style.display='block';
  }
...
function nextStep(current){
  ...
  if(current === 2){
    // For competitors this step also carries the personal fields (combined page).
    if(isComp()){
      const name = document.getElementById('f_name').value.trim();
      const email = document.getElementById('f_email').value.trim();
      const phone = document.getElementById('f_phone').value.trim();
    ...
    if(isComp()){ goToStep(4); return; }
  }
  goToStep(current+1);
}
function prevStep(current){ if(isComp() && current===4){ goToStep(1); return; } goToStep(current-1); }
...
function buildReview(){
  ...
  box.innerHTML =
    ...
    rf(L('Academic Level','المستوى الأكاديمي'), semLabel(_fv('f_semester'))) +
...
async function submitForm(){
  ...
  state.competition = null; state.mode = ''; state.flow = 'student'; // clear context after submit
  var _r=document.getElementById('htmlRoot'); if(_r) _r.classList.remove('flow-competition');
  go('success');
}
...
function resetAndGoHome(){
  state = {department:null, gender:null, flow:'student'};
  var _r=document.getElementById('htmlRoot'); if(_r) _r.classList.remove('flow-competition');
...
function semLabel(v){
  if(!v) return '';
  var m=String(v).match(/Level (\d)/);
  if(/^BS/.test(v) && m) return tr('Level','المستوى')+' '+m[1];
  if(/^PG/.test(v) && /Thesis/.test(v)) return tr('Thesis writing stage','مرحلة كتابة الرسالة');
  if(/^PG/.test(v) && m) return tr('Level','المستوى')+' '+m[1];
  return v;
}
...
var TILE_ICONS={
  ...
  mega:'<path d="M3 11v2a1 1 0 0 0 1 1h2l4 4V6L6 10H4a1 1 0 0 0-1 1zM14 8a4 4 0 0 1 0 8"/>',
  check:'<path d="M20 6L9 17l-5-5"/>'
};
function tile(cls,icon,num,label){
...
/* ---- Home: admin-controlled featured announcement popup ----
   Shows every time the home page opens/refreshes. Supports more than one
   featured item as an auto-advancing carousel. */
async function maybeShowHomePopup(){
  if(document.getElementById('homePop')) return;
  var comps=[];
  try{ comps=await getCompetitions(); }catch(e){ return; }
  var feat=comps.filter(function(c){return c.featured;});
  if(!feat.length) return;
  window._popList=feat; window._popIdx=0;
  var scrim=document.createElement('div'); scrim.className='home-pop-scrim'; scrim.id='homePop';
  scrim.onclick=function(ev){ if(ev.target===scrim) closeHomePop(); };
  scrim.innerHTML='<div class="home-pop" id="homePopCard"></div>';
  document.body.appendChild(scrim);
  renderPopCard();
  if(feat.length>1){ window._popTimer=setInterval(function(){ popNext(1); },5000); }
}
function renderPopCard(){
  var card=document.getElementById('homePopCard'); if(!card) return;
  var list=window._popList||[]; var f=list[window._popIdx]; if(!f) return;
  var title=(lang==='ar'&&f.title_ar)?f.title_ar:f.title;
  var desc=(lang==='ar'&&f.description_ar)?f.description_ar:(f.description||'');
  ...
  card.innerHTML='<span class="hp-badge">🔔 '+tr('Latest announcement','أحدث إعلان')+'</span>'
    ...
    +'<button class="btn-outline" onclick="closeHomePop()">'+tr('Close','إغلاق')+'</button></div>'+dots+'</div>';
}
function popNext(d){ var n=(window._popList||[]).length; if(!n) return; window._popIdx=((window._popIdx||0)+d+n)%n; renderPopCard(); }
function popGo(i){ window._popIdx=i; renderPopCard(); }
function closeHomePop(){ var p=document.getElementById('homePop'); if(p) p.remove(); if(window._popTimer){ clearInterval(window._popTimer); window._popTimer=null; } }
function openFeatured(id){ closeHomePop(); openCompetition(id); }
...
try{
  Object.assign(I18N.en, {
    ...
    "council.c1t":"Add Council Member","council.c1p":"Add your faculty's council members (up to 3 male and 3 female). Additions are approved by the Admin.","council.c1b":"Add Council Member",
    ...
    "f.social":"Social media accounts (optional)","f.socialadd":"+ Add account","admin.export":"Export CSV"
  });
  Object.assign(I18N.ar, {
    ...
    "council.c1t":"إضافة عضو مجلس","council.c1p":"أضِف أعضاء مجلس كليتك (بحدٍّ أقصى ٣ طلاب و٣ طالبات). تخضع الإضافات لاعتماد المشرف.","council.c1b":"إضافة عضو مجلس",
    ...
    "f.social":"حسابات التواصل الاجتماعي (اختياري)","f.socialadd":"+ إضافة حساب","admin.export":"تصدير CSV"
  });
}catch(e){}
